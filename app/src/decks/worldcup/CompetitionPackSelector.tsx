        ...
        {COMPETITIONS.map((comp) => {
          ...
          return (
            <button
              ...
              className={`group flex flex-col items-center text-left transition-transform ${!comp.live ? 'cursor-not-allowed opacity-50' : 'hover:-translate-y-1'}`}
            >
              <div className={`relative w-full ${ART_MAX_WIDTH}`}>
                {isSelected && (
                ...
                {comp.art ? (
                  <div
                    className={`aspect-[2/3] w-full overflow-hidden rounded-2xl shadow-[0_16px_32px_-20px_rgba(0,0,0,0.9)] transition-shadow ${isSelected ? 'ring-2 ring-[#8fe3b0] ring-offset-2 ring-offset-ink' : 'group-hover:shadow-[0_16px_32px_-12px_rgba(143,227,176,0.35)]'}`}
                  >
                    <img src={comp.art} alt={`${comp.label} artwork`} className="h-full w-full object-cover" />
                  </div>
                ) : (
                  <div
                    className={`flex aspect-[2/3] w-full flex-col items-center justify-center rounded-2xl border border-dashed border-paper/15 bg-paper/[0.03] px-2 text-center transition-shadow ${isSelected ? 'ring-2 ring-[#8fe3b0] ring-offset-2 ring-offset-ink' : ''}`}
                  >
